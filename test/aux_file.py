...
from test.aux_converter import human2bytes
...
class CreateTestFile:
    ...
    def __init__(self,
        ...
        if file_type == FileType.text:
            ...
            # the '_' below is for an iterable without a variable
            text = ''.join(random.choice(string.printable) for _ in range(byte_size))
            file.write(text)
            file.close()
    ...
    def set_attributes(self):
        file_attributes_calc = File.get_file_atts_calc(self.file_name)
        File.set_file_atts(self.file_name, file_attributes_calc)
    ...
    def create_checksum(self):
        ...
        checksum = File.get_file_checksum_calc(self.file_name)
        file.write(checksum)
        file.close()
